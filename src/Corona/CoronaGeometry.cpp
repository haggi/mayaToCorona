...
void CoronaRenderer::defineMesh(std::shared_ptr<MayaObject> mobj)
{
	...
	for( int mbStep = 0; mbStep < numSteps; mbStep++)
	{
		...
		{
			Logging::debug(MString("Error there is a mismatch between point data length and num vertices. MeshData points len: ") + md.points.length() + " numVertices " + numVertices);
			numSteps = 1;
			return;
		}
		if( mbStep > 0)
	...
	for (uint triId = 0; triId < numTris; triId++)
	{
		...
		if ((vtxId0 >= npts) || (vtxId1 >= npts) || (vtxId2 >= npts))
			Logging::error(MString("Index ") + vtxId0 + ";" + vtxId1 + ";" + vtxId2 + " > npts: " + npts + " -- Obj: " + obj->shortName);
